...
import { ScanItemsTemplateFilter } from "./scan-items-template-filter.table";
...
interface ScanItemsTemplateTableProps {
  columns: ColumnDef<IScanItemTemplate>[];
  data: IScanItemTemplate[];
  onDelete: (rows: Row<IScanItemTemplate>[], callback: () => void) => void;
  totalItems: number;
  currentPage: number;
  itemsPerPage: number;
...
export const ScanItemsTemplateTable = ({
  columns,
  data,
  onDelete,
  totalItems,
  ...
  onSearch,
  isLoading = false,
}: ScanItemsTemplateTableProps) => {
  const { t } = useTranslation("common");
  const [rowSelection, setRowSelection] = useState({});
  const { activeRowId, setActiveRowId } = useActiveRow("scan-items-template-table");
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className={`cursor-pointer transition-colors ${
                    activeRowId === row.original.id ? "bg-blue-50 hover:bg-blue-100" : ""
                  }`}
                >
