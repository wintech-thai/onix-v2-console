...
import { ApiKeyFilterTable } from "./apikeys-filter.table";
import {
  Select,
  SelectContent,
  SelectItem,
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => setActiveRowId((row.original as { keyId: string }).keyId)}
                  className={`cursor-pointer transition-colors ${
                    activeRowId === (row.original as { keyId: string }).keyId ? "bg-blue-50 hover:bg-blue-100" : ""
                  }`}
                >
